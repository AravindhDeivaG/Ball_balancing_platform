...
def init_setup(rec_client_id):
	...
	global servo_a_1, servo_a_2, servo_b_1, servo_b_2, vision_sensor_1

	return_code , servo_a_1 = sim.simxGetObjectHandle(client_id,'revolute_joint_ss_8',sim.simx_opmode_blocking)
	return_code , servo_a_2 = sim.simxGetObjectHandle(client_id,'revolute_joint_ss_10',sim.simx_opmode_blocking)
	return_code , servo_b_1 = sim.simxGetObjectHandle(client_id,'revolute_joint_ss_6',sim.simx_opmode_blocking)
	return_code , servo_b_2 = sim.simxGetObjectHandle(client_id,'revolute_joint_ss_4',sim.simx_opmode_blocking)
...
def control_logic(Cx,Cy,setpoint):
	"""
	Purpose:
	---
	...
	global Kp , Ki , Kd ,input_prev_x , input_prev_y , error_sum_x , error_sum_y , last_time , pid_switched_on , d_error_x,d_error_Y
	set_Cx = setpoint[0]
	set_Cy = setpoint[1]
	def set_servo_fin_position(angle_a,angle_b):
		angle_rad_a = (np.pi/180)*angle_a
		angle_rad_b = (np.pi/180)*angle_b
		sim.simxSetJointTargetPosition(client_id,servo_a_1,angle_rad_a,sim.simx_opmode_oneshot)
		sim.simxSetJointTargetPosition(client_id,servo_a_2,-angle_rad_a,sim.simx_opmode_oneshot)
		sim.simxSetJointTargetPosition(client_id,servo_b_1,angle_rad_b,sim.simx_opmode_oneshot)
		sim.simxSetJointTargetPosition(client_id,servo_b_2,-angle_rad_b,sim.simx_opmode_oneshot)

	
	
	if(pid_switched_on == False):
		error = set_Cx - Cx
		error_sum_x = error*0.25
		input_prev_x = Cx
		angle_a = Kp*error
		error = set_Cy - Cy
		error_sum_y = error*0.25
		input_prev_y = Cy
		angle_b = Kp*error
		if(angle_a>90): angle_a = 90
		elif(angle_a<-90): angle_a = -90
		if(angle_b > 90): angle_b = 90
		elif(angle_b < -90): angle_b = -90
		set_servo_fin_position(angle_a,angle_b)
		pid_switched_on = True
		last_time = time.time()


	else:
		while(time.time() - last_time < 0.15):
			pass
		else:
			last_time = time.time()
			error = set_Cx - Cx
			print(error,end = "  ")
			error_sum_x  += error*0.15
			d_error_x = (input_prev_x - Cx)/(0.15)
			angle_a = Kp*error + Kd*d_error_x + Ki*error_sum_x
			error = set_Cy - Cy
			print(error)
			error_sum_y  += error*0.15
			d_error_y = (input_prev_y - Cy)/(0.15)
			angle_b = Kp*error + Kd*d_error_y + Ki*error_sum_y
			input_prev_x = Cx
			input_prev_y = Cy
			if(angle_a>90): angle_a = 90
			elif(angle_a<-90): angle_a = -90
