...
# Team ID:			[ 2592 ]
# Author List:		[ Names of team members worked on this file separated by Comma: Aravindh Deiva G]
...
def scan_image(warped_image):
	...
	shape_dict = {}
	shape = ''
	contours , hierarchy = cv2.findContours(warped_image,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
	for cnt in contours:
		...
		if(sides>=7):
			...
			if shape in shape_dict.keys():
				shape_dict[shape].append(['None',Cx,Cy])
			else:
				shape_dict.update({shape:['None',Cx,Cy]})
		else:
			pass
	else:
		pass
