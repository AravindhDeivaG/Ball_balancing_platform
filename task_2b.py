...
# Team ID:			[ 2592 ]
# Author List:		[ DIVYA A B,sivvani M,rishivanthiya G R,aravindh deiva]
# Filename:			task_2b.py
...
def init_remote_api_server():
	...
	sim.simxFinish(-1) # closes all existing connections
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	img_temp = np.array(vision_sensor_image,np.uint8)
	img_temp = np.reshape(img_temp,(1024,1024,3))
	img_bgr = np.flip(img_temp,0)
	transformed_image = cv2.cvtColor(img_bgr,cv2.COLOR_BGR2RGB)
...
def send_data(client_id , maze_array):
	...
	array_1D = maze_array.flatten()
	return_code,retInts,retFloats,retStrings,retBuffer=sim.simxCallScriptFunction(client_id,'top_plate_respondable_1',sim.sim_scripttype_customizationscript,'receiveData',array_1D,[],[],emptyBuff,sim.simx_opmode_blocking)
